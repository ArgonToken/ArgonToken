...
ArgonToken Core version 0.11.2 is now available from:
...
ArgonToken Core's block templates are now for version 3 blocks only, and any
mining software relying on its `getblocktemplate` must be updated in parallel
to use libblkmaker either version 0.4.2 or any version from 0.5.1 onward. If you
are solo mining, this will affect you the moment you upgrade ArgonToken Core,
...
ArgonToken Core 0.11.2 implements BIP 66, which introduces block version 3, and a
new consensus rule, which prohibits non-DER signatures. Such transactions have
been non-standard since ArgonToken 0.8, but were technically still permitted
inside blocks.
...
installer (on Windows) or just copy over /Applications/ArgonToken-Qt (on Mac) or
argontokend/argontoken-qt (on Linux).
